...
#: Top-level Python packages the assembled check-environment
#: needs in its own venv. Used to filter a sub-requirements file
#: out of ``pconfig/requirements_lock.txt`` (see
#: :func:`_write_checkenv_requirements`). Downstream consumers
#: (e.g. bamaster's Dockerfile) ``pip install`` from that file
...
def generate_check_environment(projroot: str) -> None:
    ...
    _write_checkenv_requirements(projroot, out_root)
    _write_venv_setup_script(out_root)
    _write_format_stdin_script(out_root)
...
def _write_checkenv_requirements(projroot: str, out_root: str) -> None:
    """Emit a filtered requirements file for the env's own venv.

    Source: ``pconfig/requirements_lock.txt`` (the in-tree
    hash-locked manifest used by the main project). We pick out
    just the top-level packages listed in
    :data:`_CHECKENV_TOP_LEVEL_PACKAGES` and emit them as plain
    ``name==version`` lines (no hashes — pip will resolve
    transitives fresh at install time).

    Hash-locking the transitives would be more deterministic, but
    a v1 plain-text filter keeps the build step simple and the
    drift risk is bounded (only transitive-of-our-tools updates,
    which the tool authors generally control).

    Output: ``<out_root>/checkenv_requirements.txt``. Consumers
    (e.g. bamaster's Dockerfile) drop a venv at
    ``<env_root>/.venv/`` and ``pip install -r`` from this file.
    """
    import re

    lock_path = os.path.join(projroot, 'pconfig', 'requirements_lock.txt')
    if not os.path.isfile(lock_path):
        raise RuntimeError(f'Expected lock file not found: {lock_path}')

    # Each top-level entry looks like ``name==1.2.3 \`` followed by
    # one or more ``    --hash=sha256:... \`` continuation lines.
    # Match just the top-level package lines.
    pkg_re = re.compile(r'^([A-Za-z0-9_.\-]+)==([^\s\\]+)')

    found: dict[str, str] = {}
    with open(lock_path, encoding='utf-8') as infile:
        for line in infile:
            m = pkg_re.match(line)
            if m is None:
                continue
            name = m.group(1).lower()
            if name in _CHECKENV_TOP_LEVEL_PACKAGES:
                found[name] = m.group(2)

    missing = _CHECKENV_TOP_LEVEL_PACKAGES - set(found)
    if missing:
        raise RuntimeError(
            f'Could not find pinned versions for {sorted(missing)} in'
            f' {lock_path}. Update _CHECKENV_TOP_LEVEL_PACKAGES or'
            f' add the missing entries to the in-tree pins.'
        )
    ...
    body = (
        ...
        ' do not edit.\n'
        '#\n'
        '# Filtered subset of pconfig/requirements_lock.txt covering\n'
        '# the tools the checkenv venv needs. pip resolves\n'
        '# transitives fresh at install time.\n'
        + ''.join(
            f'{name}=={found[name]}\n'
            for name in sorted(_CHECKENV_TOP_LEVEL_PACKAGES)
        )
    )

    out_path = os.path.join(out_root, 'checkenv_requirements.txt')
    with open(out_path, 'w', encoding='utf-8') as outfile:
        outfile.write(body)


def _write_venv_setup_script(out_root: str) -> None:
    """Emit a small shell script that builds the env's venv idempotently.

    Both the Docker build and the dev-staging deploy flow invoke
    this script after the checkenv tree is in place. Marker-file
    gating means it's a no-op when ``checkenv_requirements.txt``
    hasn't changed — cheap to run on every deploy.

    Why a script (vs. inline ``RUN`` in the Dockerfile + parallel
    logic in the Makefile): single source of truth. The two
    consumers can't drift, and adding more setup steps later
    (extra packages, cache priming, etc.) only touches this file.
    """
    content = (
        '#!/bin/sh\n'
        '# Generated by tools/batools/checkenvironment.py —'
        ' do not edit.\n'
        '#\n'
        '# Build the checkenv\'s self-contained venv at $ENV/.venv/.\n'
        '# Idempotent: rebuilds only when'
        ' checkenv_requirements.txt changes\n'
        '# (tracked via a hash marker inside the venv).\n'
        'set -e\n'
        '\n'
        'ENV_DIR="$(cd "$(dirname "$0")" && pwd)"\n'
        'REQ="$ENV_DIR/checkenv_requirements.txt"\n'
        'VENV="$ENV_DIR/.venv"\n'
        'MARKER="$VENV/.installed-hash"\n'
        '\n'
        ...
        'rm -rf "$VENV"\n'
        'python3 -m venv "$VENV"\n'
        '"$VENV/bin/pip" install --no-cache-dir -q -r "$REQ"\n'
        'echo "$NEW_HASH" > "$MARKER"\n'
    )

    out_path = os.path.join(out_root, 'setup_venv.sh')
    with open(out_path, 'w', encoding='utf-8') as outfile:
        outfile.write(content)
    os.chmod(out_path, 0o755)


def _write_format_stdin_script(out_root: str) -> None:
    """Emit a small script that formats stdin via the env's black.

    Intended to be invoked as ``<env>/.venv/bin/python3
    <env>/_format_stdin.py`` by downstream consumers (e.g.
    bamaster's workspace-editor format handler). Keeps the boundary
    clean — consumers talk to the checkenv only via its venv
    binaries; this script is the entry point for "format text in
    the env's style", with the actual black-args definition still
    owned by ``efrotools.code.black_base_args``.
    """
    content = (
        '# Released under the MIT License. See LICENSE for details.\n'
        '#\n'
        '"""Read stdin, format via the env\'s black, write stdout.\n'
        '\n'
        'Generated by tools/batools/checkenvironment.py — do not edit.\n'
        '\n'
        'Invoked as ``<env>/.venv/bin/python3 <env>/_format_stdin.py``\n'
        'by consumers that need to format text using the checkenv\'s\n'
        'pinned ``black``. Args come from\n'
        '``efrotools.code.black_base_args``, so this stays in sync\n'
        'with ``make format`` / the ``format_files`` pcommand.\n'
        '"""\n'
        '\n'
        'from __future__ import annotations\n'
        '\n'
        ...
        '# ``efrotools`` lives in the checkenv tree, not the venv\'s\n'
        '# site-packages, so put its parent dir on sys.path before the\n'
        '# import.\n'
        "sys.path.insert(0, str(_ENV_ROOT / 'tools'))\n"
        '\n'
        '# pylint: disable=wrong-import-position\n'
        ...
        '\n'
        'def main() -> None:\n'
        '    """Format stdin via the env\'s pinned black."""\n'
        '    text = sys.stdin.read()\n'
        '    formatted = format_python_str(_ENV_ROOT, text)\n'
    ...
    out_path = os.path.join(out_root, '_format_stdin.py')
    with open(out_path, 'w', encoding='utf-8') as outfile:
        outfile.write(content)
