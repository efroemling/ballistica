...
def gen_builtin_asset_ids() -> None:
    ...
    ``.cache/asset_bundle/gui-minimal/`` and splices generated content into
    the ``// __AUTOGENERATED_BUILTIN_ASSET_IDS_*__`` autogen
    ...
    Normally invoked indirectly via ``make update`` (this is the
    one-phase-that-can-modify-checked-in-files per the build
    system design); standalone invocation exists for manual
    regen. Pass ``--check`` to fail (without writing) if either
    file would change.
    """
