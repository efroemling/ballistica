...
See ``docs/design/codegen.md`` for why outputs land in ``mgen/``.
"""
...
def collect(projroot: Path) -> BuildResult:
    ...
    Tolerant of two not-yet-resolved states so meta can complete on
    a fresh checkout where assets-cmake hasn't run yet:

    * Manifest file absent → return empty result with apverid="".
    * Manifest present but ``asset_packages`` empty → same.

    Once assets-cmake populates the cache, the lazybuild manifest
    watch re-fires meta and the generator emits the real content.
    """
...
def render_header(result: BuildResult) -> str:
    ...
    lines: list[str] = [
        ...
        '#ifndef BALLISTICA_BASE_MGEN_BUILTIN_ASSET_IDS_H_',
        '#define BALLISTICA_BASE_MGEN_BUILTIN_ASSET_IDS_H_',
        '',
        '#include <cstdint>',
        '',
    ...
    lines.extend(
        [
            '}  // namespace ballistica::base',
            '',
            '#endif  // BALLISTICA_BASE_MGEN_BUILTIN_ASSET_IDS_H_',
            '',
        ]
    )
    return '\n'.join(lines)
...
def generate(projroot: Path, check: bool = False) -> bool:
    ...
    out_dir = projroot / 'src/ballistica/base/mgen'
