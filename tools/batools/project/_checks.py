...
def check_builtin_asset_ids(self: ProjectUpdater) -> None:
    """Verify the generated builtin-asset-id splice is up to date.

    The autogen regions in ``base.h`` / ``assets.cc`` are produced by
    ``builtinassetids.generate()`` from the cached asset-package
    manifest (which in turn must match projectconfig's ``"assets"``
    pin). They're regenerated by ``assetpins`` / ``gen_builtin_asset_ids``
    — *not* by ``make update`` — so a manual projectconfig or wrapper
    change can leave the splice stale with nothing to catch it. Here we
    confirm (in check mode only) that the committed splice matches what
    the current resolved manifest would produce, failing loudly if not.
    """
    # Only meaningful when checking; ``make update`` deliberately does
    # not regenerate the splice (that lives in assetpins, which keeps
    # ``make update`` offline). And only when a manifest has been
    # resolved — env's assets-resolve lane produces it before checks
    # run; if it's absent there's nothing to validate against.
    if not self.check:
        return

    # Core-repo only. The splice is generated from a manually-managed
    # projectconfig pin here in ballistica-internal; spinoffs inherit
    # the spliced source and build their own (sometimes empty)
    # manifests, so validating splice-vs-manifest there is meaningless
    # and prone to false failures — e.g. a minimal spinoff whose
    # manifest bundles zero asset-packages, which the strict generator
    # rejects. The ``'ballistica' + 'kit'`` split stays literal while
    # the right-hand side is spinoff-substituted, so this is True only
    # in the core repo.
    if 'ballistica' + 'kit' != 'ballisticakit':
        return

    from batools import builtinassetids

    manifest_path = (
        Path(self.projroot) / '.cache/asset_bundle/gui/manifest.json'
    )
    if not manifest_path.is_file():
        return

    if builtinassetids.generate(Path(self.projroot), check=True):
        raise CleanError(
            'Generated builtin-asset-id splice in base.h / assets.cc is'
            ' out of date relative to the resolved asset-package'
            ' manifest. Run `tools/pcommand gen_builtin_asset_ids` to'
            ' regenerate it.'
        )
