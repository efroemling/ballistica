...
"""In-game UI automation helpers for the dev-build FIFO channel.
...
Emission format and logger are identical to the babase side
(``[automation] <tag> <status> <payload>`` via ``ba.app``), and the
``inspect`` helper here bridges back to :func:`babase.automation.screenshot`.
"""
...
def _click_widget(widget: object, tag: str, label: str) -> None:
    """Synthesize a click at the screen-center of the given widget."""
