...
def resolve_response(response: dui2.Response) -> None:
    ...
    # indexed; flag any full-size (resource-form) leak — some server
    # path skipped finalization. (Decode below is tolerant, so this is
    # a diagnostic, not a render gate. Local pages carry no manifest
    # and legitimately stay resource-form.)
    ...
    def _native(lstr: LangStr) -> babase.LangStr:
            ...
            if isinstance(effect, clfx.ScreenMessageV2):
                try:
                    effect.message = dataclass_from_json(
                        LangStr, _native(effect.message).to_resource_json()
                    )
                except Exception:
                    bui.uilog.exception(
...
def page_langstrs(page: dui2.Page) -> 'Iterator[LangStr]':
