...
class CaptureTheFlagGame(bs.TeamGameActivity[Player, Team]):
    ...
    def __init__(self, settings: dict):
        ...
        self._alarmsound = _classicassets.audio.alarm.get()
        self._ticking_sound = _classicassets.audio.ticking.get()
        ...
        if flag.team is team:
            ...
            if team.enemy_flag_at_base:
                ...
                self.show_zoom_message(
                    _classicassets.strings.game.name_scores(name=team.name),
                    color=team.color,
                )
                self._score(team)
        else:
            ...
            # If the home-team flag isn't here, print a message to that effect.
                ...
                if curtime - self._last_home_flag_notice_print_time > 5.0:
                    ...
                    bpos = team.base_pos
                    tval = _classicassets.strings.game.own_flag_at_base_warning
                    tnode = bs.newnode(
                        'text',
                        attrs={
                            'text': tval,
