...
class TournamentJoinInfo:
    """Display useful info for joiners."""

    def __init__(self, lobby: TournamentLobby):
        ...
        keyboard = bascenev1.getinputdevice("Keyboard", "#1", doraise=False)
...
class TournamentChooser:
    ...
    def __init__(
        ...
        self._text_node = bascenev1.newnode(
            "text",
            delegate=self,
            attrs={
    ...
    @property
    def lobby(self) -> TournamentLobby:
        """The chooser's lobby."""
        lobby = self._lobby()
        if lobby is None:
            raise babase.NotFoundError("TournamentLobby does not exist.")
        return lobby

    def get_lobby(self) -> TournamentLobby | None:
    ...
    def _set_ready(self, ready: bool) -> None:
        ...
        if not ready:
            ...
            bascenev1.getsession().handlemessage(
                RegisterTournamentMessage(self)
            )
    ...
    def _update_icon(self) -> None:
        assert babase.app.classic is not None
        if self._profilenames[self._profileindex] == "_edit":
            tex = bascenev1.gettexture("black")
            tint_tex = bascenev1.gettexture("black")
        ...
        tex = bascenev1.gettexture(tex_name)
        tint_tex = bascenev1.gettexture(tint_tex_name)
...
class TournamentLobby:
    ...
    def __init__(self) -> None:
        from bascenev1 import SessionTeam, CoopSession
        ...
        self.choosers: list[TournamentChooser] = []
    ...
    def get_choosers(self) -> list[TournamentChooser]:
        """The current choosers present."""
        return self.choosers

    def create_join_info(self) -> TournamentJoinInfo:
        ...
        """
        return TournamentJoinInfo(self)
    ...
    def add_chooser(self, sessionplayer: bascenev1.SessionPlayer) -> None:
        ...
        self.choosers.append(
            TournamentChooser(
                vpos=self._vpos, sessionplayer=sessionplayer, lobby=self
            )
        )
