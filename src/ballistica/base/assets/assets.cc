...
auto Assets::FindAssetFileCas_(FileType type, const std::string& name,
  ...
  if (type == FileType::kCubeMapTexture) {
    // Cube maps are a single faceCount=6 KTX2 blob under part 't' in the
    // package's textures bucket (shared with 2D textures; decision #24).
...
auto Assets::FindCasCubeMapTexturePath(const std::string& name) -> std::string {
  ...
  // (FileType::kCubeMapTexture), never the bucket. All profiles ship as a
  // single ``.ktx2`` per cube map (faceCount=6).
  std::string bucket_id = package_registry_.LookupTextureBucketId(apverid);
  if (bucket_id.empty()) {
    return "";
  }
...
auto Assets::LoadBundledFallbackTextureRGBA(const std::string& name)
  ...
  std::string apverid = name.substr(0, colon_pos);
  std::string logical_path = name.substr(colon_pos + 1) + ".ktx2";
  ...
  try {
    ...
    // Bucket manifest -> the asset's part-'t' data blob.
    auto bucket_doc =
    ...
    JsonRef part = bucket_doc->root()["e"][logical_path]["t"];
...
void Assets::ReloadLanguage(const std::vector<std::string>& apverids,
  ...
  for (auto&& apverid : apverids) {
    ...
    auto hash = package_registry_.LookupAssetHash(apverid, bucket_id,
                                                  "language.json", "j");
    if (hash.empty()) {
      continue;
    }
