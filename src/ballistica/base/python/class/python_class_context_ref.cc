...
void PythonClassContextRef::SetupType(PyTypeObject* cls) {
  ...
  cls->tp_doc =
      ...
      "implicitly use. Context determines, for example, which scene nodes\n"
      "or textures get added to without having to specify it explicitly in\n"
      "the newnode()/gettexture() call. Contexts can also affect object\n"
      "lifecycles; for example a babase.ContextCall will become a no-op when\n"
      "the context it was created in is destroyed.\n"
      "\n"
      ...
      "\n"
      "Creating a ContextRef will capture a reference to the current\n"
      ...
      ":meth:`babase.ContextRef.empty()` to create a reference to *no*\n"
      ...
      "=====\n"
      "\n"
      "ContextRefs are generally used with the Python ``with`` statement, "
      "which\n"
