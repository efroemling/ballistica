...
class SpazNode::FullShadowSet : public Object {
 public:
  ...
  BGDynamicsShadow torso_shadow_;
  BGDynamicsShadow head_shadow_;
  BGDynamicsShadow pelvis_shadow_;
  BGDynamicsShadow lower_left_leg_shadow_;
  BGDynamicsShadow lower_right_leg_shadow_;
  BGDynamicsShadow upper_left_leg_shadow_;
  BGDynamicsShadow upper_right_leg_shadow_;
  BGDynamicsShadow lower_left_arm_shadow_;
  BGDynamicsShadow lower_right_arm_shadow_;
  BGDynamicsShadow upper_left_arm_shadow_;
  BGDynamicsShadow upper_right_arm_shadow_;
};
...
#define BA_NODE_TYPE_CLASS SpazNode
  ...
  BA_MODEL_ATTR(head_model, head_model, set_head_model);
  BA_MODEL_ATTR(torso_model, torso_model, set_torso_model);
  BA_MODEL_ATTR(pelvis_model, pelvis_model, set_pelvis_model);
  BA_MODEL_ATTR(upper_arm_model, upper_arm_model, set_upper_arm_model);
  BA_MODEL_ATTR(forearm_model, forearm_model, set_forearm_model);
  BA_MODEL_ATTR(hand_model, hand_model, set_hand_model);
  BA_MODEL_ATTR(upper_leg_model, upper_leg_model, set_upper_leg_model);
  BA_MODEL_ATTR(lower_leg_model, lower_leg_model, set_lower_leg_model);
  BA_MODEL_ATTR(toes_model, toes_model, set_toes_model);
...
#undef BA_NODE_TYPE_CLASS
  ...
  SpazNodeType()
      : NodeType("spaz", CreateSpaz),
        ...
        torso_model(this),
        pelvis_model(this),
        upper_arm_model(this),
        forearm_model(this),
        hand_model(this),
        upper_leg_model(this),
        lower_leg_model(this),
        toes_model(this),
...
SpazNode::~SpazNode() {
  ...
  if (tick_play_id_ != 0xFFFFFFFF) {
    g_audio->PushSourceStopSoundCall(tick_play_id_);
  }
...
void SpazNode::Throw(bool with_bomb_button) {
  ...
  if (holding_something_ && !throwing_) {
    ...
    if (Sound* sound = GetRandomMedia(attack_sounds_)) {
      if (AudioSource* s = g_audio->SourceBeginNew()) {
...
void SpazNode::DoFlyPress() {
  if (can_fly_ && !knockout_ && !frozen_) {
    ...
    if (t - last_sparkle_time > 200) {
      last_sparkle_time = t;
      AudioSource* s = g_audio->SourceBeginNew();
      if (s) {
        ...
        if (r < 33) {
          s_id = SystemSoundID::kSparkle;
        } else if (r < 66) {
          s_id = SystemSoundID::kSparkle2;
        } else {
...
void SpazNode::Step() {
  ...
  if (shattered_) {
            ...
            if (since_last_punch < 300) {
              ...
              if (!knockout_ && pickup_ > 20) {
                ...
                if (!knockout_ && curse_death_time_ != 0) {
                  ...
                  float v1 = sinf(scenetime * 0.05f) * 0.12f;
                  float v2 = cosf(scenetime * 0.04f) * 0.12f;
                  ...
                  float v1 = sinf(scenetime * 0.04f) * 0.1f;
                  float v2 = cosf(scenetime * 0.03f) * 0.1f;
                  JointFixedEF* jf;
    ...
    if (can_fly_ && !dead_) {
      ...
      dBodySetPosition(b, p[0], p[1], kHappyThoughtsZPlane);
  ...
  if ((force_scream_ && scene()->time() - last_force_scream_time_ < 3000)
    ...
    if (scene()->time() - last_fall_time_ > 1000) {
      ...
      if (!(voice_play_id_ == fall_play_id_
            && g_audio->IsSoundPlaying(fall_play_id_))) {
        if (Sound* sound = GetRandomMedia(fall_sounds_)) {
          if (AudioSource* source = g_audio->SourceBeginNew()) {
            g_audio->PushSourceStopSoundCall(voice_play_id_);
            source->SetPosition(p_head[0], p_head[1], p_head[2]);
            voice_play_id_ = source->Play(sound->GetSoundData());
  ...
  if (voice_play_id_ == fall_play_id_) {
    if ((footing_ && !force_scream_)
      ...
      AudioSource* s = g_audio->SourceBeginExisting(fall_play_id_, 108);
      if (s) {
  ...
  // Update ticking.
  if (tick_play_id_ != 0xFFFFFFFF) {
    AudioSource* s = g_audio->SourceBeginExisting(tick_play_id_, 109);
    if (s) {
      s->SetPosition(p_head[0], p_head[1], p_head[2]);
      s->End();
    }
  ...
  if (have_thrown_ && scene()->time() - throw_start_ < 50) {
   ...
   // If we're knocked out, stop any mid-progress to pick up.
  if (knockout_) {
  ...
  if (fly_power_ > 20.0f && scene()->stepnum() % 3 == 1) {
    for (int i = 0; i < 1; i++) {
      ...
      e.emit_type = BGDynamicsEmitType::kFairyDust;
...
void SpazNode::DrawEyeBalls(RenderComponent* c, ObjectComponent* oc,
  ...
  if (blink_smooth_ < 0.9f) {
    ...
    c->PushTransform();
    c->TransformToBody(*body_head_);
    if (eye_scale_ != 1.0f) c->Scale(eye_scale_, eye_scale_, eye_scale_);
    c->PushTransform();
...
void SpazNode::SetupEyeLidShading(ObjectComponent* c, float death_fade,
  ...
  c->SetReflection(ReflectionType::kChar);
  c->SetReflectionScale(0.05f, 0.05f, 0.05f);
}

void SpazNode::DrawEyeLids(RenderComponent* c, float death_fade,
                           float death_scale) {
  if (!has_eyelids_ && blink_smooth_ < 0.1f) return;
...
void SpazNode::DrawBodyParts(ObjectComponent* c, bool shading, float death_fade,
  ...
  if (shading) {
    ...
    c->SetColorizeTexture(color_mask_texture_);
  ...
  if (head_model_.exists()) {
    c->DrawModel(head_model_->model_data());
  }
  c->PopTransform();
  ...
  if (hair_front_right_body_.exists()) {
    c->PushTransform();
    c->TransformToBody(*hair_front_right_body_);
    if (death_scale != 1.0f) {
      c->Scale(death_scale, death_scale, death_scale);
    }
  ...
  if (hair_ponytail_top_body_.exists()) {
    c->PushTransform();
    c->TransformToBody(*hair_ponytail_top_body_);
    if (death_scale != 1.0f) {
      c->Scale(death_scale, death_scale, death_scale);
    }
  ...
  if (hair_ponytail_bottom_body_.exists()) {
    c->PushTransform();
    c->TransformToBody(*hair_ponytail_bottom_body_);
    if (death_scale != 1.0f) {
      c->Scale(death_scale, death_scale, death_scale);
    }
  ...
  // Torso.
  c->PushTransform();
  c->TransformToBody(*body_torso_);
  if (death_scale != 1.0f) {
    c->Scale(death_scale, death_scale, death_scale);
  }
  if (torso_model_.exists()) {
    c->DrawModel(torso_model_->model_data());
  }
  c->PopTransform();

  // Pelvis.
  c->PushTransform();
  c->TransformToBody(*body_pelvis_);
  if (death_scale != 1.0f) {
    c->Scale(death_scale, death_scale, death_scale);
  }
  if (pelvis_model_.exists()) {
    c->DrawModel(pelvis_model_->model_data());
  }
  c->PopTransform();

  // Right upper arm.
  c->PushTransform();
  c->TransformToBody(*upper_right_arm_body_);
  ...
  if (upper_arm_model_.exists()) {
    c->DrawModel(upper_arm_model_->model_data());
  }
  c->PopTransform();
  ...
  if (forearm_model_.exists() && !flippers_) {
    c->DrawModel(forearm_model_->model_data());
  }
  c->PopTransform();
  if (!have_boxing_gloves_) {
    ...
    if (hand_model_.exists() && !flippers_) {
      c->DrawModel(hand_model_->model_data());
    }
  }
  c->PopTransform();
  ...
  // Right upper leg.
  c->PushTransform();
  c->TransformToBody(*upper_right_leg_body_);
  ...
  if (upper_leg_model_.exists()) {
    c->DrawModel(upper_leg_model_->model_data());
  }
  c->PopTransform();

  // Right lower leg.
  c->PushTransform();
  c->TransformToBody(*lower_right_leg_body_);
  if (death_scale != 1.0f) {
    c->Scale(death_scale, death_scale, 0.5f + death_scale * 0.5f);
  }
  if (lower_leg_model_.exists()) {
    c->DrawModel(lower_leg_model_->model_data());
  }
  c->PopTransform();

  c->PushTransform();
  c->TransformToBody(*right_toes_body_);
  if (death_scale != 1.0f) {
    c->Scale(death_scale, death_scale, death_scale);
  }
  if (toes_model_.exists()) {
    c->DrawModel(toes_model_->model_data());
  }
  c->PopTransform();
  ...
  // Left upper arm.
  c->PushTransform();
  c->TransformToBody(*upper_left_arm_body_);
  ...
  if (upper_arm_model_.exists()) c->DrawModel(upper_arm_model_->model_data());
  c->PopTransform();

  // Left lower arm.
  c->PushTransform();
  c->TransformToBody(*lower_left_arm_body_);
  c->Scale(-1, 1, 1);
  c->PushTransform();
  c->Translate(0, 0, 0.1f);
  c->Scale(1, 1, left_stretch);
  ...
  // Left upper leg.
  c->PushTransform();
  c->TransformToBody(*upper_left_leg_body_);
  ...
  if (death_scale != 1.0f)
    c->Scale(death_scale, death_scale, 0.5f + death_scale * 0.5f);
  if (upper_leg_model_.exists()) c->DrawModel(upper_leg_model_->model_data());
  c->PopTransform();

  // Lower leg.
  c->PushTransform();
  c->TransformToBody(*lower_left_leg_body_);
  ...
  // Toes.
  c->PushTransform();
  c->TransformToBody(*left_toes_body_);
  c->Scale(-1, 1, 1);
  if (death_scale != 1.0f) c->Scale(death_scale, death_scale, death_scale);
  if (toes_model_.exists()) c->DrawModel(toes_model_->model_data());
  c->PopTransform();
...
static void DrawRadialMeter(MeshIndexedSimpleFull* m, SimpleComponent* c,
                            float amt, bool flash) {
  if (flash) {
    c->SetColor(1, 1, 0.4f, 0.7f);
  } else {
    c->SetColor(1, 1, 1, 0.6f);
  }
  Graphics::DrawRadialMeter(m, amt);
  c->DrawMesh(m);
}

void SpazNode::Draw(FrameDef* frame_def) {
#if !BA_HEADLESS_BUILD

#if BA_OSTYPE_MACOS
  if (g_graphics_server->renderer()->debug_draw_mode()) {
  ...
  if (scenetime < mini_billboard_1_end_time_ && !dead_) {
    ...
    if (amt > 0.0001f && amt <= 1.0f) {
      ...
      if (!flash) {
        c.SetTexture(mini_billboard_1_texture_);
      }
      c.PushTransform();
  ...
  if (scenetime < mini_billboard_2_end_time_ && !dead_) {
    ...
    if (amt > 0.0001f && amt <= 1.0f) {
      ...
      if (!flash) c.SetTexture(mini_billboard_2_texture_);
      c.PushTransform();
  ...
  if (scenetime < mini_billboard_3_end_time_ && !dead_) {
    ...
    if (amt > 0.0001f && amt <= 1.0f) {
      ...
      bool flash = (scenetime - mini_billboard_3_start_time_ < 200
                    && render_frame_count % 6 < 3);
      if (!flash) {
        c.SetTexture(mini_billboard_3_texture_);
      }
      c.PushTransform();
      c.Translate(torso_pos[0] + 0.2f, torso_pos[1] + 1.2f,
                  torso_pos[2] - 0.2f);
  ...
  if (!name_.empty()) {
    ...
    if (explicit_bool(true)) {
      if (name_mesh_txt_ != name_) {
        ...
        name_text_group_.SetText(name_mesh_txt_, TextMesh::HAlign::kCenter,
      ...
      for (int e = 0; e < elem_count; e++) {
        ...
        TextureData* t = name_text_group_.GetElementTexture(e);
        ...
        float w = g_text_graphics->GetStringWidth(name_.c_str());
  ...
  if (billboard_opacity_ > 0.001f && !dead_) {
    ...
    c.SetTransparent(true);
    c.SetColor(1, 1, 1, o);
    c.SetTexture(billboard_texture_);
    c.PushTransform();
  ...
  // Draw life bar if our life has changed recently.
  {
    ...
    if (since_last_hurt_change < fade_time) {
      ...
      o = 1.0f - static_cast<float>(since_last_hurt_change) / fade_time;
      o *= o;
      const dReal* pos = dBodyGetPosition(body_torso_->body());
...
void SpazNode::OnGraphicsQualityChanged(GraphicsQuality q) {
  UpdateForGraphicsQuality(q);
}

void SpazNode::UpdateForGraphicsQuality(GraphicsQuality quality) {
#if !BA_HEADLESS_BUILD
...
void SpazNode::SetIsAreaOfInterest(bool val) {
  ...
  if (val && area_of_interest_ == nullptr) {
    area_of_interest_ = g_graphics->camera()->NewAreaOfInterest();
    UpdateAreaOfInterest();
  }
...
void SpazNode::SetDead(bool val) {
  ...
  if (dead_ && !was_dead) {
    ...
    if (voice_play_id_ != fall_play_id_
      ...
      if (!shattered_) {
        if (Sound* sound = GetRandomMedia(death_sounds_)) {
          if (AudioSource* source = g_audio->SourceBeginNew()) {
            const dReal* p_head = dGeomGetPosition(body_head_->geom());
            source->SetPosition(p_head[0], p_head[1], p_head[2]);
            voice_play_id_ = source->Play(sound->GetSoundData());
...
void SpazNode::SetHoldNode(Node* val) {
  ...
  if (val != nullptr) {
    ...
    {
      g_audio->PushSourceStopSoundCall(voice_play_id_);
      if (Sound* sound = GetRandomMedia(pickup_sounds_)) {
        if (AudioSource* source = g_audio->SourceBeginNew()) {
          const dReal* p_head = dGeomGetPosition(body_head_->geom());
          source->SetPosition(p_head[0], p_head[1], p_head[2]);
          voice_play_id_ = source->Play(sound->GetSoundData());
...
auto SpazNode::GetJumpSounds() const -> std::vector<Sound*> {
  return RefsToPointers(jump_sounds_);
}
void SpazNode::SetJumpSounds(const std::vector<Sound*>& vals) {
  jump_sounds_ = PointersToRefs(vals);
}
auto SpazNode::GetAttackSounds() const -> std::vector<Sound*> {
  return RefsToPointers(attack_sounds_);
}
void SpazNode::SetAttackSounds(const std::vector<Sound*>& vals) {
  attack_sounds_ = PointersToRefs(vals);
}
auto SpazNode::GetImpactSounds() const -> std::vector<Sound*> {
  return RefsToPointers(impact_sounds_);
}
void SpazNode::SetImpactSounds(const std::vector<Sound*>& vals) {
  impact_sounds_ = PointersToRefs(vals);
}
auto SpazNode::GetDeathSounds() const -> std::vector<Sound*> {
  return RefsToPointers(death_sounds_);
}
void SpazNode::SetDeathSounds(const std::vector<Sound*>& vals) {
  death_sounds_ = PointersToRefs(vals);
}

auto SpazNode::GetPickupSounds() const -> std::vector<Sound*> {
  return RefsToPointers(pickup_sounds_);
}
void SpazNode::SetPickupSounds(const std::vector<Sound*>& vals) {
  pickup_sounds_ = PointersToRefs(vals);
}

void SpazNode::SetFallSounds(const std::vector<Sound*>& vals) {
...
void SpazNode::PlayHurtSound() {
  ...
  if (Sound* sound = GetRandomMedia(impact_sounds_)) {
    if (AudioSource* source = g_audio->SourceBeginNew()) {
      const dReal* p_top = dGeomGetPosition(body_head_->geom());
      g_audio->PushSourceStopSoundCall(voice_play_id_);
      source->SetPosition(p_top[0], p_top[1], p_top[2]);
