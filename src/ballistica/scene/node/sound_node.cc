...
SoundNode::~SoundNode() {
  if (playing_) {
    g_audio->PushSourceStopSoundCall(play_id_);
  }
}
...
void SoundNode::SetVolume(float val) {
  ...
  if (playing_) {
    AudioSource* s = g_audio->SourceBeginExisting(play_id_, 106);
    if (s) {
      s->SetGain(volume_);
      s->End();
    }
...
void SoundNode::SetMusic(bool val) {
  if (val == music_) return;
  music_ = val;
  if (playing_) {
    AudioSource* s = g_audio->SourceBeginExisting(play_id_, 104);
    if (s) {
      s->SetIsMusic(music_);
      s->End();
    }
...
void SoundNode::Step() {
  ...
  if (positional_ && position_dirty_ && playing_) {
    ...
    if (t - last_position_update_time_ > 100) {
      AudioSource* s = g_audio->SourceBeginExisting(play_id_, 107);
      if (s) {
        s->SetPosition(position_[0], position_[1], position_[2]);
        s->End();
      }
