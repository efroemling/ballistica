...
PropNode::~PropNode() {
  if (area_of_interest_) {
    g_graphics->camera()->DeleteAreaOfInterest(
        static_cast<AreaOfInterest*>(area_of_interest_));
  }
}
...
void PropNode::Draw(FrameDef* frame_def) {
#if !BA_HEADLESS_BUILD

  // need our texture, model, and body to be present to draw..
  if ((!model_.exists()) || (!color_texture_.exists()) || (!body_.exists())) {
    return;
  }
  ...
  float s = model_scale_ * extra_model_scale_;
  c.Scale(s, s, s);
  c.DrawModel(model_->model_data());
  c.PopTransform();
  c.Submit();
  ...
  {  // shadow
    ...
    s_density *= 0.34f;
    {
      GraphicsQuality quality = frame_def->quality();

      // fancy new cheap shadows
      {
        float rs = shadow_size_ * model_scale_ * extra_model_scale_ * s_scale;
        float d = (quality == GraphicsQuality::kLow ? 1.1f : 0.8f) * s_density;
        g_graphics->DrawBlotch(Vector3f(pos), rs * 2.0f, 0.22f * d, 0.16f * d,
                               0.10f * d, d);
      }

      if (quality > GraphicsQuality::kLow) {
        // More sharp accurate shadow.
        if (light_model_.exists()) {
          SimpleComponent c2(frame_def->light_shadow_pass());
          c2.SetTransparent(true);
          float dd = body_type_ == BodyType::LANDMINE ? 0.5f : 1.0f;
          c2.SetColor(0.3f, 0.2f, 0.1f, 0.08f * s_density * dd);
          c2.PushTransform();
          c2.TransformToBody(*body_);
          float ss = body_type_ == BodyType::LANDMINE ? 0.9f : 1.0f;
          for (int i = 0; i < 4; i++) {
            c2.PushTransform();
            float s2 = ss * model_scale_ * extra_model_scale_
                       * (1.3f - 0.08f * static_cast<float>(i));
            c2.Scale(s2, s2, s2);
            c2.DrawModel(light_model_->model_data());
            c2.PopTransform();
          }
          c2.PopTransform();
          c2.Submit();
        ...
        if (light_model_.exists()) {
          ...
          for (int i = 0; i < 4; i++) {
            c2.PushTransform();
            float s2 = model_scale_ * extra_model_scale_ * 1.7f;
            c2.Scale(s2, s2, s2);
...
void PropNode::UpdateAreaOfInterest() {
  auto* aoi = static_cast<AreaOfInterest*>(area_of_interest_);
  if (!aoi) {
    return;
  }
  assert(body_.exists());
