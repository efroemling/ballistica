...
auto VRGraphics::ValueTest(const std::string& arg, double* absval,
...
void VRGraphics::ApplyCamera(FrameDef* frame_def) {
  Graphics::ApplyCamera(frame_def);

  CalcVROverlayMatrices(frame_def);
}

void VRGraphics::DrawWorld(Session* session, FrameDef* frame_def) {
  // Draw the standard world.
  Graphics::DrawWorld(session, frame_def);

  // Draw extra VR-Only bits.
  DrawVRControllers(frame_def);
}

void VRGraphics::DrawUI(FrameDef* frame_def) {
...
void VRGraphics::CalcVROverlayMatrices(FrameDef* frame_def) {
...
auto VRGraphics::CalcVROverlayMatrix(const Vector3f& cam_pt,
...
void VRGraphics::DrawVROverlay(FrameDef* frame_def) {
  ...
  // widgets.
  if (IsVRMode() && frame_def->overlay_flat_pass()->HasDrawCommands()) {
...
void VRGraphics::DrawOverlayBounds(RenderPass* pass) {
...
void VRGraphics::DrawVRControllers(FrameDef* frame_def) {
  if (!IsVRMode()) {
    return;
  }
