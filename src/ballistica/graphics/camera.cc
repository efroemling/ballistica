...
Camera::Camera()
    : last_mode_set_time_(GetRealTime()),
      lock_panning_(IsVRMode()),
      pan_speed_scale_(IsVRMode() ? 0.3f : 1.0f) {}
      ...
      if (explicit_bool(true)) {
        float lr_jitter;
        {
          if (IsVRMode()) {
            ...
            lr_jitter =
                sinf(static_cast<float>(GetRealTime()) / 108.0f) * 0.4f
                + sinf(static_cast<float>(GetRealTime()) / 268.0f) * 1.0f;
            lr_jitter *= 0.05f;
          }
        }
...
void Camera::Update(millisecs_t elapsed) {
  ...
  if (real_time - last_listener_update_time_ > interval) {
    ...
    if (IsVRMode()) {
#if BA_VR_MODE
      VRGraphics* vrgraphics = VRGraphics::get();
...
void Camera::ApplyToFrameDef(FrameDef* frame_def) {
  ...
  for (RenderPass** p = passes; *p != nullptr; p++) {
    ...
    (**p).SetCamera(
        ...
        up_, 4, 1000.0f,
        -1.0f,  // Auto x fov.
        final_fov_y * (g_graphics->tv_border() ? (1.0f + kTVBorder) : 1.0f),
