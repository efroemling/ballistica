...
void Renderer::PreprocessFrameDef(FrameDef* frame_def) {
  ...
  // currently in, don't try to render it.
  if (frame_def->quality() != g_graphics_server->quality()) {
...
void Renderer::VRPreprocess(FrameDef* frame_def) {
  ...
  float world_scale =
      kBaseVRWorldScale * VRGraphics::get()->vr_test_head_scale();
...
void Renderer::VRTransformToRightHand() {
  g_graphics_server->MultMatrix(vr_transform_right_hand_);
}

void Renderer::VRTransformToLeftHand() {
  g_graphics_server->MultMatrix(vr_transform_left_hand_);
}
void Renderer::VRTransformToHead() {
  g_graphics_server->MultMatrix(vr_transform_head_);
}
...
void Renderer::UpdateLightAndShadowBuffers(FrameDef* frame_def) {
  if (!light_render_target_.exists() || !light_shadow_render_target_.exists()) {
    ...
    bool high_qual = true;
    light_render_target_ = Object::MakeRefCounted(NewFramebufferRenderTarget(
    ...
    light_shadow_render_target_ = Object::MakeRefCounted(
...
void Renderer::UpdateCameraRenderTargets(FrameDef* frame_def) {
  ...
  if (frame_def->quality() >= GraphicsQuality::kHigh) {
    if (!camera_render_target_.exists()) {
      ...
      camera_render_target_ = Object::MakeRefCounted(NewFramebufferRenderTarget(
          w, h,
          true,   // linear-interp
          true,   // depth
          true,   // tex
...
void Renderer::Load() {
  screen_render_target_ = Object::MakeRefCounted(NewScreenRenderTarget());
