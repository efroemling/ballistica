...
HScrollWidget::HScrollWidget()
    : touch_mode_(!g_platform->IsRunningOnDesktop()) {
  set_draggable(false);
...
void HScrollWidget::HandleRealTimerExpired(RealTimer<HScrollWidget>* t) {
  if (touch_held_) {
...
auto HScrollWidget::HandleMessage(const WidgetMessage& m) -> bool {
  BA_DEBUG_UI_READ_LOCK;
  bool claimed = false;
  bool pass = true;
...
void HScrollWidget::Draw(RenderPass* pass, bool draw_transparent) {
  have_drawn_ = true;
