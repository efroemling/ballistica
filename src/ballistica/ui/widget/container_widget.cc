...
void ContainerWidget::SetOnActivateCall(PyObject* c) {
  on_activate_call_ = Object::New<PythonContextCall>(c);
}

void ContainerWidget::SetOnOutsideClickCall(PyObject* c) {
  on_outside_click_call_ = Object::New<PythonContextCall>(c);
}

void ContainerWidget::DrawChildren(RenderPass* pass, bool draw_transparent,
  ...
  if (background_) {
    ...
    if (single_depth_) {
      ...
      layer_thickness = 1.0f / (widgets_.size());
      layer_spacing = layer_thickness;
      base_offset = 0;
    }
  }
  ...
  if (draw_transparent) {
    ...
    for (size_t i = 0; i < w_count; i++) {
      ...
      c.PushTransform();
      float z_offs = base_offset + i * layer_spacing;
...
auto ContainerWidget::HandleMessage(const WidgetMessage& m) -> bool {
  BA_DEBUG_UI_READ_LOCK;
  ...
  switch (m.type) {
    ...
    case WidgetMessage::Type::kPaste:
      ...
      // ...reaaaly need to make ba.container() a subclass.
    ...
    case WidgetMessage::Type::kCancel: {
      if (selected_widget_) {
        if (selected_widget_->HandleMessage(m)) {
          claimed = true;
        }
      ...
      if (!claimed) {
        ...
        } else if (on_cancel_call_.exists()) {
          claimed = true;
    ...
    case WidgetMessage::Type::kTabNext:
    case WidgetMessage::Type::kMoveRight:
    case WidgetMessage::Type::kMoveDown: {
      if (m.type == WidgetMessage::Type::kTabNext && !claims_tab_) {
        break;
      }
      if (m.type == WidgetMessage::Type::kMoveRight && !claims_left_right_) {
        break;
      }
      if (m.type == WidgetMessage::Type::kMoveDown && !claims_up_down_) {
        break;
      }
      if (selected_widget_) {
        if (selected_widget_->HandleMessage(m)) {
      ...
      if (!claimed) {
        if (!root_selectable_) {
          if (m.type == WidgetMessage::Type::kMoveDown) {
            SelectDownWidget();
          } else if (m.type == WidgetMessage::Type::kMoveRight) {
            SelectRightWidget();
          } else {
            SelectNextWidget();
          }
    ...
    case WidgetMessage::Type::kTabPrev:
    case WidgetMessage::Type::kMoveLeft:
    case WidgetMessage::Type::kMoveUp: {
      if (m.type == WidgetMessage::Type::kTabPrev && !claims_tab_) {
        break;
      }
      if (m.type == WidgetMessage::Type::kMoveLeft && !claims_left_right_) {
        break;
      }
      if (m.type == WidgetMessage::Type::kMoveUp && !claims_up_down_) {
        break;
      }
      if (selected_widget_) {
        if (selected_widget_->HandleMessage(m)) {
      ...
      if (!claimed) {
        if (!root_selectable_) {
          if (m.type == WidgetMessage::Type::kMoveUp) {
            SelectUpWidget();
          } else if (m.type == WidgetMessage::Type::kMoveLeft) {
            SelectLeftWidget();
          } else {
            SelectPrevWidget();
          }
    ...
    case WidgetMessage::Type::kActivate: {
      if (root_selectable_) {
        Activate();
        claimed = true;
      } else {
    ...
    case WidgetMessage::Type::kMouseMove: {
      ...
      if (!root_selectable_) {
        ...
        for (auto i = widgets_.rbegin(); i != widgets_.rend(); i++) {
          ...
          if ((**i).HandleMessage(
                  WidgetMessage(m.type, nullptr, cx, cy, claimed))) {
            claimed = true;
          }
          if (modal_children_) {
            break;
    ...
    case WidgetMessage::Type::kMouseWheel:
    case WidgetMessage::Type::kMouseWheelH:
    case WidgetMessage::Type::kMouseWheelVelocity:
    case WidgetMessage::Type::kMouseWheelVelocityH: {
      CheckLayout();
      ...
      for (auto i = widgets_.rbegin(); i != widgets_.rend(); i++) {
        ...
        TransformPointToChild(&cx, &cy, ((**i)));
        if ((**i).HandleMessage(
                WidgetMessage(m.type, nullptr, cx, cy, amount, momentum))) {
          claimed = true;
          break;
        }
    ...
    case WidgetMessage::Type::kScrollMouseDown:
    case WidgetMessage::Type::kMouseDown: {
      ...
      if (!root_selectable_) {
        ...
        for (auto i = widgets_.rbegin(); i != widgets_.rend(); i++) {
          ...
          TransformPointToChild(&cx, &cy, **i);
          if ((**i).HandleMessage(
                  WidgetMessage(m.type, nullptr, cx, cy, click_count))) {
            claimed = true;
            break;
          }
          if (modal_children_) {
      ...
      if (!claimed && m.type == WidgetMessage::Type::kMouseDown) {
...
auto ContainerWidget::GetMult(millisecs_t current_time, bool for_glow) const
  ...
  if (root_selectable_ && selected()) {
    ...
    if (g_ui->ShouldHighlightWidgets()) {
      if (IsHierarchySelected()) {
        m = 0.5f
    ...
    if ((mouse_over_ && pressed_)
        || (current_time - last_activate_time_ < 200)) {
      m *= 1.7f;
      m2 *= 1.1f;
    } else if (g_ui->ShouldHighlightWidgets()) {
...
void ContainerWidget::SetOnCancelCall(PyObject* call_tuple) {
  on_cancel_call_ = Object::New<PythonContextCall>(call_tuple);
}
...
void ContainerWidget::Draw(RenderPass* pass, bool draw_transparent) {
  BA_DEBUG_UI_READ_LOCK;
  ...
  if (transitioning_) {
    ...
    if (!draw_transparent) {
      if (transition_type_ == TRANSITION_IN_SCALE) {
        if (net_time - dynamics_update_time_ > 1000)
          dynamics_update_time_ = net_time - 1000;
        while (net_time - dynamics_update_time_ > 5) {
          dynamics_update_time_ += 5;
          d_transition_scale_ +=
        ...
        while (net_time - dynamics_update_time_ > 5) {
          ...
          if (transitioning_) {
            millisecs_t t = dynamics_update_time_;
  ...
  if (bg_dirty_) {
    ...
    if (height > width * 0.6f) {
      tex_id = SystemTextureID::kWindowHSmallVMed;
      bg_model_transparent_i_d_ = SystemModelID::kWindowHSmallVMedTransparent;
      bg_model_opaque_i_d_ = SystemModelID::kWindowHSmallVMedOpaque;
      l_border = width * 0.07f;
      r_border = width * 0.19f;
      b_border = height * 0.1f;
      t_border = height * 0.07f;
    } else {
      tex_id = SystemTextureID::kWindowHSmallVSmall;
      bg_model_transparent_i_d_ = SystemModelID::kWindowHSmallVSmallTransparent;
      bg_model_opaque_i_d_ = SystemModelID::kWindowHSmallVSmallOpaque;
    ...
    if (w->IsSelectable()) {
      ...
      if (is_window_stack_
          && (is_overlay_window_stack_
              || !g_ui->root_widget()->overlay_window_stack()->HasChildren())) {
        w->GlobalSelect();
...
void ContainerWidget::SetTransition(TransitionType t) {
  ...
  if (t == TRANSITION_IN_SCALE) {
    transition_start_time_ = net_time;
    dynamics_update_time_ = net_time;
    transitioning_ = true;
    ...
    float screen_max_x = g_graphics->screen_virtual_width();
    float screen_max_y = g_graphics->screen_virtual_height();
...
void ContainerWidget::DeleteWidget(Widget* w) {
  ...
  if (is_overlay_window_stack_) {
    if (widgets_.empty()) {
      ...
      g_ui->root_widget()->ReselectLastSelectedWidget();
      return;
    }
  }
...
void ContainerWidget::ShowWidget(Widget* w) {
  ...
  HandleMessage(WidgetMessage(WidgetMessage::Type::kShow, nullptr, tx, ty,
                              width, height));
}
    ...
    if (!w && selected_widget_->auto_select()) {
      ...
      if (!w) {
        ...
        if (IsInMainStack()) {
          ...
          g_ui->root_widget()->ScreenPointToWidget(&x, &y);
          w = g_ui->root_widget()->GetClosestDownWidget(
              x, y, g_ui->screen_root_widget());
        }
    ...
    if (!w && selected_widget_->auto_select()) {
      ...
      if (!w) {
        ...
        if (IsInMainStack()) {
          ...
          WidgetPointToScreen(&x, &y);
          g_ui->root_widget()->ScreenPointToWidget(&x, &y);
          w = g_ui->root_widget()->GetClosestUpWidget(
              x, y, g_ui->screen_root_widget());
        }
        // When we find no viable targets for an autoselect widget we do
        // nothing.
