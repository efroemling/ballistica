...
ScrollWidget::ScrollWidget() : touch_mode_(!g_platform->IsRunningOnDesktop()) {
...
void ScrollWidget::HandleRealTimerExpired(RealTimer<ScrollWidget>* t) {
  if (touch_held_) {
...
auto ScrollWidget::HandleMessage(const WidgetMessage& m) -> bool {
  BA_DEBUG_UI_READ_LOCK;
  bool claimed = false;
  bool pass = true;
  ...
  switch (m.type) {
    ...
    case WidgetMessage::Type::kMouseWheelVelocityH: {
      if (ContainerWidget::HandleMessage(m)) {
        ...
        last_sub_widget_h_scroll_claim_time_ = GetRealTime();
      }
      pass = false;
      break;
    }
    case WidgetMessage::Type::kMouseWheelVelocity: {
      ...
      bool child_claimed_h_scroll_recently =
          (GetRealTime() - last_sub_widget_h_scroll_claim_time_ < 100);
        ...
        if (touch_mode_) {
          ...
          if (!touch_is_scrolling_) {
            ...
            child_is_scrolling_ = ContainerWidget::HandleMessage(
                WidgetMessage(WidgetMessage::Type::kScrollMouseDown, nullptr,
            ...
            if (!child_is_scrolling_) {
              touch_delay_timer_ =
                  Object::New<RealTimer<ScrollWidget>>(150, false, this);
            }
          }
        }
          ...
          if (touch_held_) {
            ...
            if (child_is_scrolling_ && !child_disowned_scroll_) {
              bool move_claimed = ContainerWidget::HandleMessage(
                  WidgetMessage(WidgetMessage::Type::kMouseMove, nullptr,
                                m.fval1, m.fval2, m.fval3));
              // If they stopped claiming them, send a scroll-mouse-up to tie
              // things up.
              if (!move_claimed) {
                ContainerWidget::HandleMessage(
                    WidgetMessage(WidgetMessage::Type::kMouseUp, nullptr,
    ...
    case WidgetMessage::Type::kMouseUp: {
      ...
      if (touch_mode_) {
        if (touch_held_) {
          ...
          if (child_is_scrolling_ && !child_disowned_scroll_) {
            ContainerWidget::HandleMessage(
                WidgetMessage(WidgetMessage::Type::kMouseUp, nullptr, m.fval1,
                              m.fval2, false));
          }
          ...
          if (!claimed2 && !touch_down_sent_) {
            ContainerWidget::HandleMessage(WidgetMessage(
      ...
      if (!((x >= 0.0f) && (x < width() + right_overlap) && (y >= 0.0f)
        ...
        pass = false;
        ContainerWidget::HandleMessage(WidgetMessage(
            WidgetMessage::Type::kMouseUp, nullptr, m.fval1, m.fval2, true));
      }
  ...
  if (m.type == WidgetMessage::Type::kMouseDown && claimed) {
    GlobalSelect();
  }
...
void ScrollWidget::Draw(RenderPass* pass, bool draw_transparent) {
  have_drawn_ = true;
  ...
  if (!draw_transparent) {
    ...
    while (current_time - inertia_scroll_update_time_ > 5) {
      ...
      if (!has_momentum_
          && (current_time - last_velocity_event_time_ > 1000 / 30))
        inertia_scroll_rate_ = 0;
    ...
    // scroll thumb at depth 0.8f-0.9
    {
      ...
      if (draw_transparent) {
        c.DrawModel(g_media->GetModel(
            sb_thumb_height > 100
                ? SystemModelID::kScrollBarThumbTransparent
                : SystemModelID::kScrollBarThumbShortTransparent));
      } else {
        c.DrawModel(g_media->GetModel(
            sb_thumb_height > 100 ? SystemModelID::kScrollBarThumbOpaque
                                  : SystemModelID::kScrollBarThumbShortOpaque));
      }
      c.PopTransform();
      c.ScissorPop();
      c.Submit();
  ...
  if (draw_transparent && IsHierarchySelected()
      && g_ui->ShouldHighlightWidgets() && highlight_) {
    float m = 0.8f
