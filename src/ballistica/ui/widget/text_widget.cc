...
void TextWidget::set_on_return_press_call(PyObject* call_tuple) {
  on_return_press_call_ = Object::New<PythonContextCall>(call_tuple);
}

void TextWidget::set_on_activate_call(PyObject* call_tuple) {
  on_activate_call_ = Object::New<PythonContextCall>(call_tuple);
}
...
void TextWidget::Draw(RenderPass* pass, bool draw_transparent) {
  ...
  // If we're on a button or something, add tilt.
  {
    ...
    Vector3f tilt = tilt_scale * g_graphics->tilt();
    l -= tilt.y;
    r -= tilt.y;
    b += tilt.x;
    t += tilt.x;
  ...
  if (editable() || IsSelectable()) {
    ...
    if ((IsSelectable()
         ...
         && (always_highlight_ || g_ui->ShouldHighlightWidgets()))
  ...
  TextMesh::HAlign align_h;
  TextMesh::VAlign align_v;

  switch (alignment_h_) {
    case HAlign::kLeft:
      x_offset = l;
      align_h = TextMesh::HAlign::kLeft;
      break;
    case HAlign::kCenter:
      x_offset = (l + r) * 0.5f;
      align_h = TextMesh::HAlign::kCenter;
      break;
    case HAlign::kRight:
      x_offset = r;
      align_h = TextMesh::HAlign::kRight;
      break;
    default:
  ...
  switch (alignment_v_) {
    case VAlign::kTop:
      y_offset = t;
      align_v = TextMesh::VAlign::kTop;
      break;
    case VAlign::kCenter:
      y_offset = (b + t) * 0.5f;
      align_v = TextMesh::VAlign::kCenter;
      break;
    case VAlign::kBottom:
      y_offset = b;
      align_v = TextMesh::VAlign::kBottom;
      break;
    default:
    ...
    if (IsHierarchySelected() || always_show_carat_) {
      ...
      if (show_cursor
          && ((current_time / 100) % 2 == 0
              || (current_time - last_carat_change_time_ < 250))) {
...
void TextWidget::BringUpEditDialog() {
  ...
  if (explicit_bool(use_internal_dialog)) {
    g_python->LaunchStringEdit(this);
  }
}
...
auto TextWidget::HandleMessage(const WidgetMessage& m) -> bool {
  if (HeadlessMode()) {
    return false;
  }
    ...
    case WidgetMessage::Type::kMouseDown: {
      ...
      if ((x >= (-left_overlap)) && (x < (width_ + right_overlap))
        ...
        if (c && IsSelectable()) {
          ...
          if (editable()) {
            if (InputDevice* kb = g_input->keyboard_input()) {
              g_ui->SetUIInputDevice(kb);
            }
          }
          GlobalSelect();
          pressed_ = true;
...
auto TextWidget::GetTextWidth() -> float {
  ...
  return g_text_graphics->GetStringWidth(text_translated_, big_);
}
