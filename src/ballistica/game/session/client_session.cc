...
// How many buckets we keep around for calculating max-lag-spikes.
const int kDelayBufferBuckets = 20;

ClientSession::ClientSession()
    : base_time_(0),
      least_buffered_count_list_(kDelayBufferBuckets, 0),
      most_buffered_count_list_(kDelayBufferBuckets, 0),
      adjust_counter_(0),
      buffer_count_list_index_(0),
      steps_on_list_(0),
      current_cmd_ptr_(nullptr),
...
void ClientSession::OnReset(bool rewind) {
  ClearSessionObjs();
  target_base_time_ = base_time_ = 0;
}
...
void ClientSession::Update(int time_advance) {
  ...
  target_base_time_ += static_cast<double>(time_advance) * correction_;
  ...
  try {
    ...
    while (base_time_ < target_base_time_) {
      ...
      if (!commands_.empty()) {
        ...
        // Any time we run out of commands we immediately pull our target time
        // back to where we currently are at. We want to stay *behind* the
        // "buffering line".
      ...
      switch (cmd) {
        case SessionCommand::kBaseTimeStep: {
          ...
          steps_on_list_ -= stepsize;
          BA_PRECONDITION(steps_on_list_ >= 0);
          base_time_ += stepsize;
          break;
        }
        ...
        case SessionCommand::kAddNode: {
          ...
          int id = vals[2];
          if (id < 0 || id > 10000) throw Exception("invalid node id");
...
void ClientSession::HandleSessionMessage(const std::vector<uint8_t>& buffer) {
  ...
  switch (buffer[0]) {
    case BA_MESSAGE_SESSION_RESET: {
      ...
      // just a session-command. (Do we not want it added to replays?...)
      Reset(false);
      break;
    }
    ...
    case BA_MESSAGE_SESSION_COMMANDS: {
      ...
      while (true) {
        ...
        subBuffer.resize(size);
        memcpy(&(subBuffer[0]), &(buffer[offset + 2]), subBuffer.size());
        AddCommand(subBuffer);
        offset += 2 + size;  // move to next command
        if (offset == buffer.size()) {
          // lets also use this opportunity to graph our command-buffer size for
          // network debugging.. if (NetGraph *graph =
    ...
    case BA_MESSAGE_SESSION_DYNAMICS_CORRECTION: {
      ...
      std::vector<uint8_t> bufferOut = buffer;
      bufferOut[0] = static_cast<uint8_t>(SessionCommand::kDynamicsCorrection);
      AddCommand(bufferOut);
      break;
    }
