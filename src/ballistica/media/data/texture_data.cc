...
static void rgba8888_unpremultiply_in_place(void* src, size_t cb) {
...
TextureData::TextureData(TextPacker* packer) : packer_(packer) {
  file_name_ = packer->hash();
  valid_ = true;
}

TextureData::TextureData(const std::string& qr_url) : is_qr_code_(true) {
  file_name_ = qr_url;
  valid_ = true;
}
...
void TextureData::DoPreload() {
  ...
  TextureQuality texture_quality = g_graphics_server->texture_quality();
  ...
  if (packer_.exists()) {
    ...
    void* tex_ref{g_platform->CreateTextTexture(
        width, height, strings, positions, visible_widths, scale)};
    uint8_t* pixels{g_platform->GetTextTextureData(tex_ref)};
    ...
    g_platform->FreeTextTexture(tex_ref);

    // Downsample this down to rgba4444 in-place.
    TexturePreloadData::rgba8888_to_rgba4444_in_place(buffer, buffer_size);
    ...
    if (type_ == TextureType::k2D) {
      ...
      if (file_name_size > 12
        ...
        assert(g_graphics_server->SupportsTextureCompressionType(
        ...
        if (((preload_datas_[0].formats[preload_datas_[0].base_level]
            ...
            && (!g_graphics_server->SupportsTextureCompressionType(
