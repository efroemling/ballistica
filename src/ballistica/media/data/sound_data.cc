...
static auto LoadOgg(const char* file_name, std::vector<char>* buffer,
  ...
  if (corrupt) {
    ...
    if (!reported_corrupt) {
      ...
      g_python->PushObjCall(Python::ObjID::kPrintCorruptFileErrorCall);
    }
...
static void LoadCachedOgg(const char* file_name, std::vector<char>* buffer,
  ...
  if (g_platform->Stat(file_name, &stat_ogg) == 0) {
    ogg_mod_time = stat_ogg.st_mtime;
  }
  FILE* f_cache = g_platform->FOpen(cache_file_name.c_str(), "rb");
  ...
  if (success) {
    FILE* f = g_platform->FOpen(cache_file_name.c_str(), "wb");
    bool success2 = false;
    if (f) {
      ...
      if (!success2) {
        g_platform->Unlink(cache_file_name.c_str());
      }
    }
  }
}
...
SoundData::SoundData(const std::string& file_name_in)
