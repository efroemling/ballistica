...
static void CalcERPCFM(dReal stiffness, dReal damping, dReal* erp, dReal* cfm) {
...
class BGDynamicsServer::Terrain {
 public:
  Terrain(BGDynamicsServer* t, Object::Ref<CollideModelData>* collide_model_in)
      : collide_model_(collide_model_in) {
    assert((**collide_model_).loaded());
    geom_ = dCreateTriMesh(nullptr, (**collide_model_).GetBGMeshData(), nullptr,
                           nullptr, nullptr);
  }

  auto GetCollideModel() const -> CollideModelData* {
    return collide_model_->get();
  }

  ~Terrain() {
    dGeomDestroy(geom_);
    // We were passed an allocated pointer to a CollideModelData strong-ref
 ...
 private:
  Object::Ref<CollideModelData>* collide_model_;
  dGeomID geom_;
};
...
class BGDynamicsServer::Field {
 public:
  ...
  auto birth_time() const -> uint32_t { return birth_time_; }
  auto lifespan() const -> dReal { return lifespan_; }
...
class BGDynamicsServer::Tendril {
 public:
  struct Point {
    ...
    float glow_g{};
    float glow_b{};
    void Update(const Tendril& t) {
      p += v * kGameStepSeconds;
      age += kGameStepMilliseconds;
      v *= 0.992f;
      v.y -= 0.003f * bouyancy;     // Bouyancy.
      v.x += 0.005f * t.wind_amt_;  // Slight side drift.
      erode *= (1.0f - 0.06f * erode_rate);
      if (age > 750 * fade_rate) fade *= 1.0f - 0.0085f * fade_rate;
    }
    void UpdateDistortion(const BGDynamicsServer& d) {
      ...
      for (auto&& fi : d.fields_) {
        ...
        if (dist_squared <= fRadSquared) {
          ...
          float falloff =
              (1.0f - (dist / fRad));  // falloff with dist from field
          mag *= falloff;
...
void BGDynamicsServer::UpdateTendrils() {
  ...
  for (auto i = tendrils_.begin(); i != tendrils_.end();) {
    ...
    if (t.controller_ == nullptr) {
      ...
      t.position_ += t.velocity_ * kGameStepSeconds;
    }
...
void BGDynamicsServer::PushRemoveTerrainCall(CollideModelData* collide_model) {
  PushCall([this, collide_model] {
    ...
    for (auto i = terrains_.begin(); i != terrains_.end(); ++i) {
      if ((**i).GetCollideModel() == collide_model) {
        found = true;
        delete *i;
        terrains_.erase(i);
        break;
    ...
    if (draw_shadow || draw_light) {
      ...
      if (shadow_dist > -kShadowOccludeDistance
        ...
        if (sd <= 0.0f) {
          ...
          density = 0.5f * g_graphics->GetShadowDensity(m[12], m[13], m[14])
...
void BGDynamicsServer::Step(StepData* step_data) {
  ...
  graphics_quality_ = g_graphics_server->graphics_quality_requested();
  ...
  time_ += kGameStepMilliseconds;  // milliseconds per step
...
void BGDynamicsServer::PushAddTerrainCall(
  ...
  PushCall([this, collide_model] {
    ...
    terrains_.push_back(new Terrain(this, collide_model));
...
void BGDynamicsServer::UpdateFields() {
  ...
  while (i != fields_.end()) {
    ...
    {
      bool kill = false;
      if (static_cast<float>(time_ - f.birth_time()) > f.lifespan()) {
        kill = true;
      }
      if (kill) {
        auto i_next = i;
    ...
    float age = static_cast<float>(time() - f.birth_time()) / f.lifespan();
...
void BGDynamicsServer::UpdateChunks() {
  ...
  for (auto i = chunks_.begin(); i != chunks_.end();) {
    ...
    {
      bool kill = false;
      if (static_cast<float>(time_ - c.birth_time_) > c.lifespan_) {
        kill = true;
      }
