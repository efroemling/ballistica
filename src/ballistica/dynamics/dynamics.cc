...
void Dynamics::process() {
  in_process_ = true;
  real_time_ = GetRealTime();  // Update this once so we can recycle results.
  ProcessCollisions();
          ...
          for (auto context : contexts) {
            for (auto&& i : context->skid_sounds) {
              if (c->skid > i.target_impulse * clip) {
                ...
                if (i.playing) {
                  AudioSource* s = g_audio->SourceBeginExisting(i.play_id, 101);
                  if (s) {
                ...
                if (i.playing) {
                  g_audio->PushSourceFadeOutCall(i.play_id, 200);
                  i.playing = false;
                }
              }
            }
          ...
          for (auto context : contexts) {
            for (auto&& i : context->roll_sounds) {
              if (c->roll > i.target_impulse * clip) {
                ...
                // and position; otherwise get a sound started.
                if (i.playing) {
                  AudioSource* s = g_audio->SourceBeginExisting(i.play_id, 102);
                  if (s) {
                    s->SetGain(volume * i.volume);
                    s->SetPosition(apx, apy, apz);
                    s->End();
